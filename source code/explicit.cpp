...
{
  int w = 1024, h = 768, samps = argc == 2 ? atoi(argv[1]) / 4 : 8;  // # samples
    ...
    for (unsigned short x = 0, Xi[3] = { 0, 0, y *y *y
      ...
      for (int sy = 0, i = (h - y - 1) *w + x; sy < 2; sy++)  // 2x2 subpixel rows
        for (int sx = 0; sx < 2; sx++, r = Vec())
        {
          ...
          {
            double r1 = 2* srand(Xi), dx = r1 < 1 ? sqrt(r1) - 1 : 1 - sqrt(2 - r1);
            double r2 = 2* srand(Xi), dy = r2 < 1 ? sqrt(r2) - 1 : 1 - sqrt(2 - r2);
  ...
  FILE *f = fopen("explicit.ppm", "w"); // Write image to PPM file.
  fprintf(f, "P3\n%d %d\n%d\n", w, h, 255);
  for (int i = 0; i < w * h; i++)
